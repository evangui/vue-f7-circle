	...
	<div class="page">
		...
		<div class="toolbar">
		 ...
		 <div class="page-content infinite-scroll-content" @infinite="loadMoreComments">
			<div class="comments">
				<div  class="comment-title">主评论</div>
				<div class="comment-list">
					<div v-for="item in mainComment" class="comment-item comment-row">
				...
				<div class="comment-list">
					<div v-for="item in allComments" class="comment-item comment-row">
						...
						<div class="detail">
							...
							<div class="time"><span>{{item.reply_addtime|friendlytime}}</span></div>
							<div @click="atCommentAuthor(item.reply_id, item.member_name)" class="text">
								<span v-if="item.replied_member_id && item.replied_replyid!=replyID" class="text-color-green">回复 {{item.replied_member_name}}：</span>
...
<script>
	...
	export default {
		...
		created() {
			document.title = '文章评论';
			var self = this;
			self.replyID = self.$route.params.id;
		},
		...
		mounted() {
			...
			//新评论
//			wap.getCache(this.newCommentsKeyPre + self.layer1_obj_id);
...
//				self.$$('.sheet-modal').hide();
		...
		methods: {
			...
			loadMainComment: function() {
				...
				this.$wap.getajax('api/user/articlereply/searchList', data, function(res) {
					if(res.status == 'success') {
						self.articleID = res.data.datalist[0].layer1_obj_id;
						self.mainComment = res.data.datalist;
					}
				}, 'v1', true, 7200);
			},
...
//		    	$('.editor-container textarea').attr('placeholder', '@'+authorName);
		    },
		    removeAt() {
		    	this.atAuthor = {
		    		id: 0,
...
//				self.$f7.preloader.show("评论发布中...");
				...
				this.$wap.doajax('api/user/articlereply/save', data, function(res) {
					...
					if(res.status == 'success') {
						...
						self.$itemCacheFlag('article-reply-sub', self.replyID, false);
						self.allComments = [];
...
//					self.$f7.preloader.hide();
			...
			loadAllComments: function(page) {
				...
				let cacheTime = 0;
				if (self.$itemCacheFlag('article-reply-sub', self.replyID)) {
					cacheTime = 0;
				} else {
					cacheTime = 1800;
				}
